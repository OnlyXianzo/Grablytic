#!/usr/bin/env bash
#
# TrueStream universal Linux installer — one command, zero decisions:
...
# and verifies the `truestream` binary. Test hooks: OS_RELEASE_FILE, UNAME_M,
# DRY_RUN=1 (print actions without executing).
#
set -euo pipefail

REPO="OnlyXianzo/TrueStream"
VERSION="${VERSION:-latest}"   # or: VERSION=0.0.1 ... | bash
DRY_RUN="${DRY_RUN:-0}"
OS_RELEASE_FILE="${OS_RELEASE_FILE:-/etc/os-release}"
UNAME_M="${UNAME_M:-$(uname -m)}"

log()  { printf '[truestream] %s\n' "$*"; }
...
Usage: install.sh [--version X.Y.Z] [--uninstall] [--dry-run] [--help]
  ...
  --uninstall      remove truestream with the native package manager
  --dry-run        print every action without executing (also: DRY_RUN=1)
  --help           this text
One-liner: curl -fsSL https://raw.githubusercontent.com/OnlyXianzo/TrueStream/main/install.sh | bash
EOF
}
...
if command -v apt-get >/dev/null 2>&1 && [[ "$ID_LIKE_LOWER" == *debian* || "$ID_LIKE_LOWER" == *ubuntu* ]]; then
  PM="apt"; PM_INSTALL="sudo apt-get install -y"; PM_REMOVE="sudo apt-get remove -y truestream"
elif command -v dnf >/dev/null 2>&1; then
  PM="dnf"; PM_INSTALL="sudo dnf install -y"; PM_REMOVE="sudo dnf remove -y truestream"
elif command -v zypper >/dev/null 2>&1; then
  PM="zypper"; PM_INSTALL="sudo zypper --non-interactive install"; PM_REMOVE="sudo zypper --non-interactive remove truestream"
elif command -v pacman >/dev/null 2>&1; then
  PM="pacman"; PM_INSTALL="sudo pacman -U --noconfirm"; PM_REMOVE="sudo pacman -Rns --noconfirm truestream"
else
...
# --- uninstall path -----------------------------------------------------------
if [ "$UNINSTALL" = "1" ]; then
  log "removing truestream via $PM..."
  run "$PM_REMOVE"
  log "done."
  exit 0
fi
...
case "$PM" in
  apt)    FILE="truestream_${VER}_${DEB_ARCH}.deb" ;;
  dnf|zypper) FILE="truestream-${VER}-1.${RPM_ARCH}.rpm" ;;
  pacman) FILE="truestream-${VER}-1-${PKG_ARCH}.pkg.tar.zst" ;;
esac
URL="$(pick_url "^${FILE//./\\.}\$")"
[ -n "$URL" ] || { log "no $PM asset named $FILE in $TAG"; exit 1; }
log "asset: $FILE"
...
command -v truestream >/dev/null 2>&1 && log "installed: $(command -v truestream)" || { log "install finished but 'truestream' not on PATH"; exit 1; }
