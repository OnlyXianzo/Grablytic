...
def main() -> int:
    ...
    out.append("---")
    out.append("")
    out.append("## ✨ What's New & User Benefits")
    out.append("")
    out.append("- 🏎️ **Unstoppable Download Queues:** Cancelling active downloads or changing maximum concurrent downloads in Settings no longer stalls the download queue. Remaining items start immediately without hanging.")
    out.append("- 🔄 **Smart Incomplete Download Resume:** If a download was interrupted, Grablytic now searches both your internal cache and your custom download folder. You can resume `.part` files right where you left off without wasting bandwidth.")
    out.append("- 🧹 **Automatic Storage Hygiene:** Deleting a media download from your library now cleans up orphan thumbnail sidecars (`.jpg`, `.webp`), saving device storage while keeping shared artwork intact.")
    out.append("- 📱 **Rock-Solid Background Downloads on Android:** Upgraded WakeLock and WifiLock management ensures your downloads keep running in the background when the screen turns off or while switching apps.")
    out.append("- ⚡ **Faster Playlist & Media Extraction:** Standardized Node/QuickJS runtime execution solves YouTube signature challenges and protects against UI freezing on large playlists (up to 500 items).")
    out.append("- 🛡️ **Hardened System Security:** Replaced prefix string matching with strict path containment (`commonpath`) to guarantee downloads and cleanup operations stay strictly within designated folders.")
    out.append("")
    out.append("---")
    out.append("")
    out.append("## 🌟 Deep Technical Highlights & Architectural Upgrades")
    out.append("")
