...
Auto-filing is gated by ``TRUESTREAM_AUTO_REPORT=1`` — anonymous by default.
"""
...
DEFAULT_REPO = os.environ.get("GITHUB_REPO", "OnlyXianzo/TrueStream")
TOKEN_ENV = "GITHUB_TOKEN"
AUTO_REPORT_ENV = "TRUESTREAM_AUTO_REPORT"
...
def notify_exception(exc: BaseException, *, context: dict | None = None,
    ...
    ``TRUESTREAM_AUTO_REPORT=1``; otherwise returns ``skipped`` after logging.
    """
...
def main(argv: list[str]) -> int:
    """Manual trigger: ``python -m truestream_engine.github_notifier --title "…"``."""
    import argparse
    ap = argparse.ArgumentParser(description="File a TrueStream issue from server logs.")
    ap.add_argument("--title", required=True)
    ap.add_argument("--summary", default="")
    ap.add_argument("--repo", default=DEFAULT_REPO)
    args = ap.parse_args(argv)
    try:
        from truestream_engine.persistent import read_log_tail, sanitize
